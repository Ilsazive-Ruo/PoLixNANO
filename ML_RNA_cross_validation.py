...
if os.path.exists('weight/' + experiment_name):
    shutil.rmtree('weight/' + experiment_name)
...
for train_index, test_index in skf.split(y, x):
    ...
    joblib.dump(model, 'weight/' + experiment_name + '/' + experiment_name + '_' + str(k) + '.pkl')
    temp = []
...
df = pd.DataFrame(r2)
df2 = pd.DataFrame(FI)
df2.to_csv('weight/' + experiment_name + '/' + experiment_name + '_FI.csv', index=False)
